...
# La funcion de arrojar un resultado aleatorio entre 2 valores desde la biblioteca "random"
...
# Para solicitar el nombre de los jugadores
def obtener_resultados_jugadores(jugadores, numero_tiros):
    resultados_jugadores = []
    for i in range(jugadores):
        nombre = input(f"Ingrese el nombre del jugador {i + 1}: ")
        # Para que todos los jugadores tengan la misma cantidad de tiradas
...
def imprimir_resultados(resultados_jugadores):
    for nombre, resultados in resultados_jugadores:
        ...
        print(f"El jugador {
              nombre} obtuvo los siguientes resultados: {resultados}")
...
def jugar():  # En esta función se llama a las demas funciones auxiliares en el orden correcto para que funcione el "juego"
    ...
    numero_tiros = int(
        input("Ingrese el número de tiros para todos los jugadores: "))
    resultados_jugadores = obtener_resultados_jugadores(
        jugadores, numero_tiros)
    imprimir_resultados(resultados_jugadores)
    # Ordenar por puntaje de mayor a menor
    resultados_jugadores.sort(key=lambda x: sum(x[1]), reverse=True)
